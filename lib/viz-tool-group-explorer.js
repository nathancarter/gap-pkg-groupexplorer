    ...
    */
    ...
    const hue2color = ( hue ) => {
        ...
        return json.data.visualization == 'CayleyDiagram' ?
            `hsl(${angle},53%,30%)` : `hsl(${angle},100%,80%)`;
    }
    ...
    window.addEventListener( 'message', function ( event ) {
        if ( event.data == 'listener ready' ) {
            // compute some data we need in many cases below
            ...
            if ( copy.partition ) {
                const whichPart = g =>
                    copy.partition.indexOf(
                        copy.partition.find( part =>
                            part.indexOf( g ) > -1 ) );
                const whichColor = g =>
                    hue2color( whichPart( g )
                             / copy.partition.length );
                console.log( copy.partition );
                console.log( groupElts.map( whichPart ) );
                console.log( groupElts.map( whichColor ) );
                iwin.postMessage( {
                    json : {
                        highlights : {
