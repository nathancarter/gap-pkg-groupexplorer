...
//     and will be ignored.
//   subgroup : a list of natural numbers, indices into the group, of a
//     set of elements that should be highlighted in the resulting
//     visualization (if just one group is being shown).  technically, this
//     can be any subset, not just a subgroup, but typically it's a subgroup.
...
function ( element, json, callback ) {
    ...
    */
    ...
    // Prepare data for highlighting a subgroup or a partition.
    ...
    const whichPart = g => copy.partition.indexOf(
        copy.partition.find( part => part.indexOf( g ) > -1 ) );
    const whichColor = g =>
        hue2color( whichPart( g ) / copy.partition.length );
    ...
    if ( copy.subgroup2 && copy.multtable2 ) {
        highlights2 = {
            background : groupElts2.map( g =>
                copy.subgroup2.indexOf( g ) > -1 ?
    ...
    if ( copy.partition ) {
        highlights = {
            background : groupElts.map( whichColor )
        };
    }
